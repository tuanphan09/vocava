...
public class SpeechService extends Service {
        ...
        public void onNext(RecognizeResponse response) {
            ...
            if (response.getResultsCount() > 0) {
                for (int i = 0; i < response.getResultsCount(); i++) {
                    final SpeechRecognitionResult result = response.getResults(i);
                    if (result.getAlternativesCount() > 0) {
                        final SpeechRecognitionAlternative alternative = result.getAlternatives(0);
                        text = alternative.getTranscript();
                        Log.d("AppLog", text);
    ...
    private class AccessTokenTask extends AsyncTask<Void, Void, AccessToken> {
        ...
        OnAccessTokenTaskCompleted listener;

        public AccessTokenTask(OnAccessTokenTaskCompleted listener) {
        ...
        protected AccessToken doInBackground(Void... voids) {
            ...
            // ***** WARNING *****
            // In this sample, we load the credential.json from a JSON file stored in a raw resource
